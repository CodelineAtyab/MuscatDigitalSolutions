...
> **A `> *` rule on a grid card must not reach vanilla-tilt's glare.** The library appends `.js-tilt-glare` as the card's last child, so `.ai-card > * { grid-column: 1 / -1 }` gave it a *definite* grid placement — and an absolutely-positioned grid child with a definite placement resolves its containing block to the grid **area**, which sits inside the card's padding, rather than the padding box. The sheen stopped 17–29 px short of the left and right edges (`.ai-card` 172 px of a 218 px card, `.path-card` 522 of 580, `.reason-card`/`.team-card` 244 of 278) while the ungridded `.service-card` and `.step` stayed correct at 1 px, so the card read as two differently lit surfaces meeting a mismatched border. Only the inline axis was wrong, since `grid-row` stayed `auto`. `.tilt-card .js-tilt-glare { grid-area: auto }` restores the padding box in both axes; verify with `glare.getBoundingClientRect()` insetting exactly 1 px (the border) on all four sides of the card.

The residual 1 px is correct and invisible: the glare covers the padding box, and the `::before` gradient ring — `inset: 0`, `z-index: 3`, so it paints over the glare — occupies exactly that border ring at full opacity whenever the sheen is showing. Do not `!important` the library's inline `top`/`left`/`width`/`height` to chase it.
